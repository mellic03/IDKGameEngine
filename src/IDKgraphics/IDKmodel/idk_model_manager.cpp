...
idk::ModelManager::_load_mtl( std::string raw_mtl )
{
    ...
    while (getline(stream, line))
    {
        ...
        else if (line.find("map_Kd") != std::string::npos)
        {
            ...
            _materials.get(material_id).albedo_texture = _texture_gl_IDs[line + "-srgb"];
        }
        ...
        else if (line.find("map_Ks") != std::string::npos)
        {
            size_t root = line.find("assets/");
            line = line.substr(root);
            _materials.get(material_id).specular_texture = _texture_gl_IDs[line];
        }
    }
}
...
idk::ModelManager::loadOBJ( std::string raw_obj, std::string raw_mtl )
{
    ...
    // Position
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(idk::Vertex), (void *)offset);
    glEnableVertexAttribArray(0);
    offset += 3 * sizeof(float);

    // Normal
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(idk::Vertex), (void *)offset);
    glEnableVertexAttribArray(1);
    offset += 3 * sizeof(float);

    // UV
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(idk::Vertex), (void *)offset);
...
void
idk::ModelManager::addTexture( std::string name, GLuint gl_id, int w, int h, uint8_t* &data )
{
    // int texfile_id = _texfiles.add( { w, h, data } );
    // _texfile_IDs[name] = texfile_id;
    _texture_gl_IDs[name] = gl_id;
}
